...
class TMCMotorActor(MessageHandler):
    ...
    def __init__(self, name: str, port: Union[int, str], motorDict: Optional[dict[str, int]] = None,
                 **kwargs) -> None:
    ...
    # Motor methods
    def _get_motor_number(self, motor: Union[int, str]) -> int:
    ...
    def move_by_units(self, motor: Union[int, str], difference: float,
        ...
        try:
            ...
            self.move_by(motor, difference, velocity)
    ...
    def _get_voltage(self) -> float:
        return self.get_analog_input(8) / 10

    def _voltage_check(self) -> None:
        if (voltage := self._get_voltage()) < 10:
            raise SystemError(f"System voltage is below the threshold of 10 V: {voltage} V.")
